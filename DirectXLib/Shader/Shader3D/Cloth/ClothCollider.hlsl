...
//過去の位置に向かって球との距離と球の半径分押し出す
float3 extrudeSphere(float3 now_p1, float3 last_p1, float3 now_p2, float hit_size) {
	float dist = distance(now_p1, now_p2);
	float3 vec = subtract(last_p1, now_p1);
	vec = normalize(vec);
...
void ClothCollider(uint3 th_id : SV_GroupID) {
	...
	for (int ite = 0; ite < size; ite++) {
		...
		//	}
		//}
		if (isHitSphereAndLine(in_vert[id1].pos, last_vert[id1].pos, in_vert[id2].pos, hit_dist)) {
			float3 f = extrudeSphere(in_vert[id1].pos, last_vert[id1].pos, in_vert[id2].pos, hit_dist);
			out_spring[id1].force = add(out_spring[id1].force, f);
		}
	}
